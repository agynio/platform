...
TerminateResponse
- New type exported for signaling termination: `export class TerminateResponse { constructor(public message?: string) {} }`.
...
Finish tool
- New tool providing a standard way to end a task.
  - Name: `finish`
...
- Three static config fields were added to SimpleAgent:
...
Enforcement node
- New node injects a SystemMessage with `restrictionMessage` when the model attempts to finish without tool calls.
