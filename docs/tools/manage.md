...
- `removeWorker(agent: AgentNode)`: unregisters the agent by instance reference, so renaming the agent after connection is supported.
...
```
 ...
 Behavior
- list: returns `string[]` of connected worker titles (the agent’s current `config.title` values).
