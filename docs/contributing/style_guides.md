...
const key = process.env.LITELLM_MASTER_KEY; // string | undefined
...
const Config = z.object({ LITELLM_MASTER_KEY: z.string().min(1) });
...
export const client = new OpenAI({ apiKey: cfg.LITELLM_MASTER_KEY });
```
