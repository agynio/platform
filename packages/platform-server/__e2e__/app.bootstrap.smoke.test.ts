...
describe('App bootstrap smoke test', () => {
  it('initializes Nest application and wires critical dependencies', async () => {
    ...
    try {
      ...
      const notificationsPublisher = app.get(NotificationsPublisher);
      expect(notificationsPublisher).toBeInstanceOf(NotificationsPublisher);
      expect(subscriptionSpy).toHaveBeenCalledTimes(1);
      const [listener] = subscriptionSpy.mock.calls[0] ?? [];
      expect(typeof listener).toBe('function');

      const cleanupRegistry = Reflect.get(notificationsPublisher as object, 'cleanup') as unknown;
      ...
      const configService = ConfigService.register(
        new ConfigService().init(
          configSchema.parse({
            ...
            dockerRunnerConnectProbeIntervalMs: 5,
            dockerRunnerConnectRetryBaseDelayMs: 2,
