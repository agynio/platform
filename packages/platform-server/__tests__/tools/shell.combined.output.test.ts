...
function createToolWithContainer(container: ContainerHandle) {
  ...
  const node = new ShellCommandNode(envService as any, logger as any, {} as any, archiveStub as any, runEvents as any, eventsBus as any);
  node.setContainerProvider({
    provide: async () => container,
  } as any);

  return { tool: node.getTool(), runEvents, eventsBus };
}
...
describe('ShellCommandTool combined output', () => {
  ...
  it('returns stderr-only output when exit code is zero', async () => {
    ...
    expect(terminalArgs).toMatchObject({ status: 'success', bytesStdout: 0, bytesStderr: Buffer.byteLength('git warning', 'utf8') });
  });
