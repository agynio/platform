...
- Threads persist per-channel metadata (`Thread.channel` JSON). Migration `20251106121500_thread_channel_json` adds this column.
 - Outbound Slack messages via the universal `send_message` tool are routed through the SlackTrigger that originated the thread. The trigger holds credentials in its configuration (bot_token via plaintext or Vault ref). `ChannelInfo.meta.triggerNodeId` is used for routing.
 - The legacy `send_slack_message` tool is kept for backward compatibility but deprecated. It does NOT route via trigger; instead it owns its token resolved from the node configuration (`bot_token` supports plaintext or Vault ref) with a fallback to `SLACK_BOT_TOKEN` environment variable.

Slack credentials
- Configure bot token in SlackTrigger node configuration (supports Vault ref) for `send_message` routing. For the legacy `send_slack_message` tool, configure the token on the tool node (`bot_token`) or set `SLACK_BOT_TOKEN` in environment. Do not store secrets in Thread.channel.
