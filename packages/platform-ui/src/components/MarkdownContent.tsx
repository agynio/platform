...
const markdownComponents: Components = {
  ...
  code: ({ inline, className, children }: MarkdownCodeProps) => {
    ...
    return (
      <code className="bg-[var(--agyn-bg-light)] text-[var(--agyn-purple)] px-1.5 py-0.5 rounded text-sm break-all">
        {children}
      </code>
    );
  },
  ...
  // Horizontal rule
  hr: () => <hr className="border-0 border-t border-[var(--agyn-border-subtle)] my-6" />,
  ...
  thead: ({ children }) => <thead className="bg-[var(--agyn-bg-light)]">{children}</thead>,
  ...
  // Text emphasis
  strong: ({ children }) => <strong className="text-[var(--agyn-dark)]">{children}</strong>,
  em: ({ children }) => <em className="text-[var(--agyn-dark)]">{children}</em>,
  del: ({ children }) => <del className="text-[var(--agyn-gray)] opacity-70">{children}</del>,
};
