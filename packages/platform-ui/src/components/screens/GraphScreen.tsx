        ...
        <div className="flex-1 relative bg-[var(--agyn-bg-light)] overflow-hidden">
          ...
          <div className="relative w-full h-full p-8">
            ...
            <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 text-center pointer-events-none">
              <div className="text-2xl font-semibold text-[var(--agyn-text-subtle)] mb-2">Graph Canvas</div>
              <div className="text-sm text-[var(--agyn-text-subtle)]">Click on nodes to view their properties</div>
            </div>
            ...
            {nodes.map((node) => (
              ...
              >
                <Node kind={node.kind} title={node.title} selected={selectedNodeId === node.id} />
              </div>
            ))}
          </div>
        </div>
