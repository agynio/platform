...
export function AICheckpointItem({
  item,
  parsed,
  kindBadge,
  rawToggleButton,
        ...
        parsed.toolCalls && parsed.toolCalls.length > 0 && (
          <div className="space-y-1">
            {parsed.toolCalls.map((tc, i) => {
              const callId = (tc.raw as any)?.id || (tc.raw as any)?.function_call_id || ''; // eslint-disable-line @typescript-eslint/no-explicit-any
              return (
                <div key={i} className="rounded border border-border/50 bg-card/40">
                  <details className="group">
                    <summary className="flex cursor-pointer items-center justify-between px-2 py-1 font-mono text-[10px] list-none">
                      <span className="truncate">{tc.name} {callId && `(${callId})`}</span>
